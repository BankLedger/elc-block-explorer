...
<section data-ng-controller="AddressController" data-ng-init="findOne()">
  <div class="secondary_navbar hidden-xs hidden-sm" scroll data-ng-class="{'hidden': !secondaryNavbar}" data-ng-show="address.addrStr" data-ng-init="hideSNavbar=0">
    <div class="container" data-ng-if="!hideSNavbar">
      ...
      <div class="col-md-4">
        <span class="txvalues txvalues-primary"><strong translate>Final Balance</strong> {{$root.currency.getConvertion(address.balanceSat) || address.balanceSat + ' ELC' }}</span>
      </div>
    </div>
  ...
  <h1><span translate>地址</span> <small data-ng-show="address.addrStr">{{$root.currency.getConvertion(address.balanceSat) || address.balanceSat + ' ELC'}}</small></h1>
  ...
  <div data-ng-if="address.addrStr">
    ...
    <h2 translate>摘要 <small>确定</small></h2>
    <div class="row" data-ng-hide="!address.addrStr">
      <div class="col-md-10">
        <table class="table">
          <tbody>
          <tr>
            <td><strong translate>总接收</strong></td>
            <td class="ellipsis text-right">{{$root.currency.getConvertion(address.totalReceivedSat) || address.totalReceivedSat + ' ELC'}}</td>
          </tr>
          <tr>
            <td><strong translate>总发送</strong></td>
            <td class="ellipsis text-right">{{$root.currency.getConvertion(address.totalSentSat) || address.totalSentSat + ' ELC'}}</td>
          </tr>
          <tr>
            <td><strong translate>账户余额</strong></td>
            <td class="ellipsis text-right">{{$root.currency.getConvertion(address.balanceSat) || address.balanceSat + ' ELC'}}</td>
          </tr>
           <tr>
            <td><strong translate>可用余额</strong></td>
            <td class="ellipsis text-right">{{$root.currency.getConvertion(address.unspendValue) || address.unspendValue + ' ELC'}}</td>
          </tr>
           <tr>
            <td><strong translate>锁定额度</strong></td>
            <td class="ellipsis text-right">{{$root.currency.getConvertion(address.lockedValue) || address.lockedValue + ' ELC'}}</td>
          </tr>
          <tr>
