        ...
        <div class="panel panel-default" data-ng-repeat="vin in tx.vinSimple| startFrom:currentInNoExpanded*sizeInNoExpanded | limitTo:sizeInNoExpanded">
          <div class="panel-body transaction-vin-vout">
            <div class="pull-right btc-value" data-ng-class="{'text-danger': $root.currentAddr == vin.addr}">
              {{$root.currency.getConvertion(vin.value) || vin.value + ' ELC'}}
            </div>
            <div class="ellipsis">
          ...
          <div class="transaction-vin-vout panel panel-default" data-ng-hide="vout.addr.indexOf('Unparsed address') > -1">
            ...
            <div class="panel-body">
              <div class="pull-right btc-value" data-ng-class="{'text-success': $root.currentAddr == vout.addr}">
                ...
                <span>ELC </span>
               	<img src="{{vout.isLocked}}" style="top:3px;color:#666;"/>
...
<div class="line-top row" data-ng-hide="!tx">
  <div class="col-xs-12 col-sm-12 col-md-12">
  		...
  		<span data-ng-show="tx.isCoinBase" class="txvalues txvalues-default">
			<span data-ng-show="block.height > 5000" translate>奖励: {{$root.currency.getConvertion(tx.fees) || tx.fees + ' ELC'}}</span>  
			<span data-ng-show="block.height <= 5000" translate>奖励: {{$root.currency.getConvertion(tx.valueOut.toFixed(8)) || tx.valueOut.toFixed() + ' ELC'}} </span> 
			<span data-ng-show="!block.height" translate>奖励: {{$root.currency.getConvertion(tx.valueOut.toFixed(8)) || tx.valueOut.toFixed(8) + ' ELC'}} </span> 
		</span>

		<span data-ng-show="tx.isCoinstake" class="txvalues txvalues-default">
			<span data-ng-show="tx.fees" translate>奖励: {{$root.currency.getConvertion(tx.fees) || tx.fees + ' ELC'}}  </span>
		</span>

		<span data-ng-show="!tx.isCoinstake && !tx.isCoinBase && tx.fees > 0" class="txvalues txvalues-default">
			<span data-ng-show="tx.fees" translate>费用: {{$root.currency.getConvertion(tx.fees) || tx.fees + ' ELC'}}  </span>
		</span>
	    <span class="txvalues txvalues-primary" style="float:right" >{{$root.currency.getConvertion(tx.valueOut.toFixed(8)) || tx.valueOut.toFixed(8) + ' ELC' }}</span>
	    <span>&nbsp;&nbsp;</span>
