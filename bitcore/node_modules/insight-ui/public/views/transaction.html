    ...
    <div class="container" data-ng-if="!hideSNavbar">
      ...
      <div class="col-md-6 col-lg-5 text-right">
        <span data-ng-show="tx.confirmations" class="txvalues
          ...
          <span translate>确认</span>
        </span>
        <span data-ng-show="!tx.confirmations" 
          class="txvalues txvalues-danger" translate>未确认交易!</span>
        <span class="txvalues txvalues-primary">{{$root.currency.getConvertion(tx.valueOut) || tx.valueOut + ' ELC' }}</span>
      </div>
    </div>
  ...
  <div data-ng-if="tx.txid">
    ...
    <div class="progress progress-striped active" data-ng-if="!tx.txid">
      <div class="progress-bar progress-bar-info" style="width: 100%">
        <span translate>加载交易详情</span> <span class="loader-gif"></span>
      </div>
    </div>
    <div data-ng-if="tx.txid">
      <div class="well well-sm ellipsis">
        <strong translate>交易</strong> 
      ...
      <table class="table" style="table-layout: fixed">
        <tbody>
        <tr>
          <td><strong translate>大小</strong></td>
          <td class="text-muted text-right">{{tx.size}} (bytes)</td>
        </tr>
        <tr data-ng-show="tx.fees">
          <td><strong translate>费用率</strong></td>
          <td class="text-muted text-right">{{$root.currency.getConvertion((tx.fees * 1000) / tx.size) + ' per kB' || ((tx.fees * 1000) / tx.size) + 'ELC per kB'}}</td>
        </tr>
        <tr>
          <td><strong translate>收到时间</strong></td>
          <td data-ng-show="tx.time" class="text-muted text-right">{{tx.time * 1000|date:'yyyy-MM-dd HH:mm:ss'}}</td>
          <td data-ng-show="!tx.time" class="text-muted text-right">N/A</td>
        </tr>
        <tr>
          <td><strong translate>开采时间</strong></td>
          <td data-ng-show="tx.blocktime" class="text-muted text-right">{{tx.blocktime * 1000|date:'yyyy-MM-dd HH:mm:ss'}}</td>
          <td data-ng-show="!tx.blocktime" class="text-muted text-right">N/A</td>
        </tr>
        <tr>
          <td><strong translate>来自区块</strong></td>
          <td data-ng-show="tx.blockhash" class="text-muted text-right"> 
            <div class="ellipsis">
              <a href="block/{{tx.blockhash}}">{{tx.blockhash}}</a>
            </div>
          <td data-ng-show="!tx.blockhash" class="text-muted text-right">未确认</td>
        </tr>
