...
var MDGAT = require('./MongoDBGetAddressTransaction');
var UnlockTransactions = require('./UnlockTransaction');
...
Bitcoin.prototype.getAPIMethods = function() {
  var methods = [
    ...
    ['getBlockByRealHash',this,this.getBlockByRealHash,1],
    ['getBlockHashByHeight',this,this.getBlockHashByHeight,1],
    ...
    ['getBlockByNumber', this, this.getBlockByNumber, 1],   
    ['getSpentInfo', this, this.getSpentInfo, 1],
    ['getInfo', this, this.getInfo, 0],
    ['getBlockJsonByHash', this, this.getBlockJsonByHash, 0],   
    ['syncPercentage', this, this.syncPercentage, 0],
    ['isSynced', this, this.isSynced, 0],
    ['getRawTransaction', this, this.getRawTransaction, 1],
    ['getRawTransactionJson', this, this.getRawTransactionJson, 1],
...
Bitcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  var configPath = path.resolve(spawnOptions.datadir, './elcoin.conf');
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.MDGAT=new MDGAT(self,callback);         
          self.genesisBuffer = blockBuffer;
          //self.emit('ready');
          self.MDGAT.synchroizedBlockAdressTxToMongoDB();
          
          self.unlockTxs=new UnlockTransactions(self);
        });
      });
  ...
  function finish() {
    ...
    if (txids && !rangeQuery) {
      ...
      self.client.getAddressTxids(txidOpts, function(err, response) {
        ...
        if (err) {
        	 console.log("self.client.getAddressTxids:"+err.message);
          return callback(self._wrapRPCError(err));
        }
 ...
 * @param {Function} callback
 */
Bitcoin.prototype.getAddressHistoryOld = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
...
Bitcoin.prototype.getAddressHistory = function(addressArg, options, callback) {
  ...
  db.address.find({'address':addressArg}).toArray(function(err,addressList){
    ...
    if(addressList && addressList[0]){
    	txids = addressList[0].txs.reverse();
    }
    var totalCount = txids.length;
    try {
      txids = self._paginateTxids(txids, fromArg, toArg);
...
Bitcoin.prototype.getAddressSummary = function(address, options, callback) {
	
	var self = this; 
	var summary = {};
	var summaryTxids = [];

	db.address.find({'address':address}).toArray(function(err,addressObj){
		if(err){
			callback(err);
		}
		if(addressObj && addressObj[0]){
			summaryTxids = addressObj[0].txs;
			summary.appearances = addressObj[0].txs.length;
	        summary.totalReceived = parseFloat(addressObj[0].Received ? addressObj[0].Received : 0);
	        summary.totalSpent = parseFloat(addressObj[0].Sented ? addressObj[0].Sented : 0);
	        summary.balance = summary.totalReceived -summary.totalSpent;
	        summary.lockedValue=addressObj[0].lockedValue?parseFloat(addressObj[0].lockedValue):0;
	      
	    	var fromArg = parseInt(options.from || 0);
	    	var toArg = parseInt(options.to || summaryTxids.length);
		    if ((toArg - fromArg) > summaryTxids.length) {
		      return callback(new Error(
		    ...
		    var paginatedTxids;
		    try {
		      paginatedTxids = self._paginateTxids(summaryTxids, fromArg, toArg);
...
Bitcoin.prototype._maybeGetBlockHash = function(blockArg, callback) {
  ...
  if (_.isNumber(blockArg) || (blockArg.length < 40 && /^[0-9]+$/.test(blockArg))) {
    self._tryAllClients(function(client, done) {
      client.getBlockByNumber(blockArg, function(err, response) {
	  if (err) {
          return done(self._wrapRPCError(err));
        }
    	
        done(null, response.result.hash);
      });
    }, callback);
  } else {
  ...
  function queryBlock(err, blockhash) {
    ...
    self._tryAllClients(function(client, done) {
      self.client.getBlock(blockhash, false, function(err, response) {
        ...
        var buffer = new Buffer(JSON.stringify(response.result), 'hex');
...
Bitcoin.prototype.getBlockHashByHeight = function(height, callback) {
	  // TODO apply performance patch to the RPC method for raw data
	  var self = this;
	  self._tryAllClients(function(client, done) {
	      self.client.getBlockByNumber(height, false, function(err, response) {    	  	
	        if (err) {
	          return done(self._wrapRPCError(err));
	        }
	        var rightHash=response.result.hash;
	        var result={"hash":rightHash};
	        done(null, result);  
	      });
	    }, callback);
}
...
Bitcoin.prototype.getBlockJsonByHash = function(hash, callback) {

	  var self = this;
	  self._tryAllClients(function(client, done) {
	      self.client.getBlock(hash, true, function(err, response) {    	  	
	        if (err) {
	          return done(self._wrapRPCError(err));
	        }	      
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = null;
          try{
        	  blockObj=  bitcore.Block.fromString(response.result);
          }catch( e){     	        
        	  client.getBlock(blockhash, true, function(err, response) {
        		  try{
        			  blockObj=  bitcore.Block.fromObject(response.result);
        			  if(blockObj){
        			  ...
        			  var height=response.result.height;
        			  client.getBlockByNumber(height, function(err, response) {
        				 
        				  var rightHash=response.result.hash;
        				  
        				  client.getBlock(rightHash, false, function(err, response) {
        					  if (err) {
        				            return done(self._wrapRPCError(err));
        				        }
        					  try{
        						  blockObj=  bitcore.Block.fromString(response.result);
        						  if(blockObj){
        							  var myReturnObject={value:blockObj,hash:rightHash};
        							  done(null, myReturnObject);
        						  }
        					  }
        					  catch(e){
...
/**
 * get for each input txid 
 * get rawTransaction to get the value index value ......
 */
Bitcoin.prototype.fillTxInputValueToTx = function(result,vin, callback) {
	  var self = this;
	 
	  self._tryAllClients(function(client, done) {

		    async.map(vin,function(inputItem,callback){
		        if(inputItem.coinbase){
		        	var newInputItem = inputItem;  
		        	callback(null,newInputItem); 
		        }else{
		        	client.getRawTransaction(inputItem.txid, 1,function(err, response) {
				        if (err) {
				          return done(self._wrapRPCError(err));
				        }		        
				        inputItem.value=response.result.vout[inputItem.vout].value;
				    	var _setValue = inputItem;  
...
Bitcoin.prototype.getDetailedTransaction = function(txid,callback) {
  ...
  function isLocked(unlockHeight,currBlockMaxHeight,txBlockHeight) {	
  ...
  //
  function isEverLocked(unlockHeight,txBlockHeight) {	
		if(unlockHeight && unlockHeight>0){
  ...
  function getMyMany(out,tx) {	
		if(isLocked(out.unlockHeight,tx.blockMaxHightNow,tx.txBlockHeight)){
			  var monthDouble =( out.unlockHeight - tx.txBlockHeight) / (60*24*30);
			  var monthInt = parseInt(monthDouble.toFixed(0));
			return parseFloat(240*out.value) / parseFloat(monthInt + 240);
		}
		else if(isEverLocked(out.unlockHeight,tx.txBlockHeight)){
			  var monthDouble =( out.unlockHeight - tx.txBlockHeight) / (60*24*30);
			  var monthInt = parseInt(monthDouble.toFixed(0));
			return parseFloat(240*out.value) / parseFloat(monthInt + 240);
		}else{
  ...
  function addOutputsToTx(tx, result) {
    ...
    for(var outputIndex = 0; outputIndex < result.vout.length; outputIndex++) {
      ...
      out.value=getMyMany(out,tx);
      tx.outputSatoshis += out.value;
      var address = null;
      ...
      client.getRawTransaction(txid, 1, function(err, response) {
        ...
        self.getInfo(function(err,info){
          if(err){
      	  ...
      	  tx.blockMaxHightNow=info.blocks;
      	  if(!result.blockhash){
      		  
      		  tx.txBlockHeight=info.blocks+1;
     		  if (result.vin[0] && result.vin[0].coinbase) {
     	          tx.coinbase = true;
     	      }
     	      else if(result.vin.length>0 &&  result.vout.length>=2 && parseFloat(result.vout[0].value)===0 &&
     	        		(  (!result.vout[0].scriptPubKey) || ( result.vout[0].scriptPubKey.asm==='' && result.vout[0].scriptPubKey.type==='nonstandard') ) ){
     	        	 tx.coinstake = true;
     	      }
     	      ...
     	      if (tx.coinbase) {
     	           tx.feeSatoshis = 0;
     	      } else if(tx.coinstake){
     	          tx.feeSatoshis = tx.outputSatoshis - tx.inputSatoshis;
     	      }else {
     	    	  
     	          tx.feeSatoshis = tx.inputSatoshis - tx.outputSatoshis;
     	      }
     	      try{
     	          tx.feeSatoshis=parseFloat(tx.feeSatoshis).toFixed(8);
     	      }
     	      catch(e){
     		  ...
     		  self.transactionDetailedCache.set(txid, tx);
      		    ...
      		    self.getBlockJsonByHash(result.blockhash,function(err,rs){
		      		  if(err){
		      			  done(err);	      			 
		              }
		      		  else{
			      			  tx.txBlockHeight=rs.height;
				      		  if (result.vin[0] && result.vin[0].coinbase) {
				      	          tx.coinbase = true;
				      	      }
				      	      else if(result.vin.length>0 &&  result.vout.length>=2 && parseFloat(result.vout[0].value)===0 &&
